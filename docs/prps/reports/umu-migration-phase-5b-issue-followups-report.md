...
| 1.6 | Extend `probe_flatpak_host_umu_candidates` with Faugus Launcher paths        | [done] Complete | Deviation: `is_flatpak` param injected instead of env-touching at test time (explained below)                                                         |
| 2.1 | Extend `readiness.rs` with Steam-Deck caveats + Faugus + `apply_*_dismissal` | [done] Complete | +8 tests including 16-row matrix equivalents                                                                                                          |
...
- **Task 1.6 injected `is_flatpak: bool` as a parameter** to `flatpak_host_umu_candidates` rather than touching `FLATPAK_ID` in the environment during tests. Reason: mutating process env across parallel tests is racy and broke an existing test. The production `probe_flatpak_host_umu_candidates` wrapper reads `env::var_os("FLATPAK_ID")` once and passes it in — production behavior is preserved, tests stay environment-isolation-safe.
- **Task 2.1 added a `FAUGUS_TRUST_LINE` const and applied it via `format!`** across all distro branches instead of duplicating the string. Also exported `apply_steam_deck_caveats_dismissal` through `onboarding/mod.rs` for consumer use — a small additive re-export, no semantic change.
...
- **Clippy `uninlined_format_args` warning** appeared briefly in `readiness.rs` after Task 2.1's `format!("{}\n{}", description, FAUGUS_TRUST_LINE)` pattern. Task 2.1's agent caught and fixed it inline (converted to `format!("{description}\n{FAUGUS_TRUST_LINE}")`). Task 2.2/2.3/2.4 agents all flagged it in their validation notes; all subsequent clippy runs are clean.
...
| `src/crosshook-native/crates/crosshook-core/src/onboarding/readiness.rs`   | 8 new | caveats present/absent matrix, Faugus substring, dismissal clear/noop, `all_passed` invariance  |
| `src/crosshook-native/crates/crosshook-core/src/launch/runtime_helpers.rs` | 3 new | Faugus candidate probe (includes/excludes/picks-up-tempdir)                                     |
