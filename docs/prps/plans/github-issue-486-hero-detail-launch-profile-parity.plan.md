...
Classification: **Ported** (already in Hero Detail) / **Port** (this plan) / **Superseded** (redundant in game-scoped Hero Detail — documented, not built) / **Deferred(#N)** (owned elsewhere). Finalized against the shipped UI in Task 5.2.
...
| Launch action (game) — in place                                                          | `LaunchPanelControls` via `launchGame` (`LaunchPanel.tsx:158`)         | `onLaunch` navigates to legacy `/launch` (`LibraryPage.tsx:170-175`) | **Port** (Task 3.1, in-place via `LaunchStateContext`)                                     |
| Launch trainer action                                                                    | `LaunchPanel.tsx:96,150,159`                                           | Missing                                                              | **Port** (Task 3.1)                                                                        |
| `onBeforeLaunch` dep-gate interception                                                   | `LaunchPage.tsx:215`; `LaunchPanel.tsx:92-103`                         | Missing — `onLaunch` fires directly                                  | **Port** (Task 3.1)                                                                        |
| Dependency gate modal (prefix deps)                                                      | `LaunchDepGateModal` + `useLaunchDepGate` (`LaunchPage.tsx:76-80,219`) | Missing                                                              | **Port** (Task 3.1)                                                                        |
| Launch feedback / diagnostic report + copy JSON                                          | `LaunchPanelFeedback` (`LaunchPanel.tsx:107-143`)                      | Missing                                                              | **Port** (Task 3.1)                                                                        |
| Version-status acknowledge ("Mark as Verified")                                          | `LaunchPanelVersionStatus` (`LaunchPanel.tsx:176-180`)                 | Missing                                                              | **Port** (Task 3.2 — via `useProfileActions` in Profiles tab)                              |
| Launch pipeline visualization                                                            | `LaunchPipeline` (`LaunchPanel.tsx:165`)                               | Missing                                                              | **Port** (Task 3.1, render in Launch tab feedback area)                                    |
| Helper log path indicator                                                                | `LaunchPanel.tsx:166-168`                                              | Missing                                                              | **Port** (Task 3.1)                                                                        |
| Launch guidance / hint text                                                              | `LaunchPanel.tsx:169-173`                                              | Missing                                                              | **Port** (Task 3.1)                                                                        |
...
| ProtonDB lookup + env guidance                                                           | `ProtonDbLookupCard` (`EnvironmentTabContent.tsx:82-99`)               | Missing                                                              | **Port** (Task 2.1)                                                                        |
| ProtonDB overwrite confirmation                                                          | `ProtonDbOverwriteConfirmation` (`EnvironmentTabContent.tsx:101-108`)  | Missing                                                              | **Port** (Task 2.1)                                                                        |
| ProtonDB suggestion accept/dismiss                                                       | `LaunchPage.tsx:55-74,208-210`                                         | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Launch optimizations toggles                                                             | `OptimizationsTabContent.tsx:50-63` (`LaunchOptimizationsPanel`)       | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Optimization preset selector (named presets)                                             | `OptimizationsTabContent.tsx:54-57`                                    | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Bundled optimization presets (one-click apply)                                           | `OptimizationsTabContent.tsx:58-59`                                    | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Save manual preset                                                                       | `OptimizationsTabContent.tsx:61`                                       | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Gamescope config (game) + method gating                                                  | `GamescopeTabContent.tsx:33-37` (`GamescopeConfigPanel`)               | Missing                                                              | **Port** (Task 2.1)                                                                        |
| "Inside gamescope session" guard                                                         | `LaunchPage.tsx:156` via `depGate.isGamescopeRunning`                  | Missing                                                              | **Port** (Tasks 2.1 + 3.1)                                                                 |
| MangoHud config + gating                                                                 | `MangoHudTabContent.tsx:36-41` (`MangoHudConfigPanel`)                 | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Steam Launch Options preview                                                             | `SteamOptionsTabContent.tsx:34-38`                                     | Missing                                                              | **Port** (Task 2.1)                                                                        |
| Offline readiness panel + launch-path warnings + trainer-hash actions                    | `OfflineTabContent.tsx:50-101`                                         | Overview shows passive `offlineReport` only                          | **Port** (Task 2.1 — Offline sub-tab with auto-switch)                                     |
| Offline auto-switch on concern                                                           | `LaunchSubTabs.tsx:82-98` (`OFFLINE_AUTO_SWITCH_PATTERN`)              | Missing                                                              | **Port** (Task 2.1 — comes free with `LaunchSubTabs`)                                      |
| Merged autosave chip (opts/gamescope/mangohud)                                           | `useAutoSaveChip` (`LaunchSubTabs.tsx:69-124`)                         | Env-only feedback                                                    | **Port** (Task 2.1 — comes with `LaunchSubTabs`)                                           |
| Full `LauncherExport` panel (status, preview script/desktop, delete, re-export)          | `ProfileSubTabs.tsx:287-306`                                           | Inline `.desktop` export button only                                 | **Port** (Task 3.2 — full panel in Profiles editor; Launch tab keeps the inline button)    |
...
| Runner method select (steam_applaunch / proton_run / native)               | `ProfileSubTabs.tsx:201` (`RunnerMethodSection`)                           | Missing — method is derived/read-only                                | **Port** (Task 2.2)                                                                |
| Trainer editor (path/type/loading mode/network isolation/version set)      | `ProfileSubTabs.tsx:244-252` (`TrainerSection` + `TrainerVersionSetField`) | Missing — Trainer top-level tab is read-only                         | **Port** (Task 2.2, decision 1)                                                    |
| Trainer-gamescope config (+derived-from-game notice)                       | `ProfileSubTabs.tsx:258-284` (`GamescopeConfigPanel`)                      | Missing                                                              | **Port** (Task 2.2)                                                                |
| Game metadata bar                                                          | `ProfileSubTabs.tsx` (`GameMetadataBar`)                                   | Missing                                                              | **Port** (Task 2.2)                                                                |
| Launcher export (full panel incl. `pendingReExport`)                       | `ProfileSubTabs.tsx:287-306` (`LauncherExport`)                            | Missing                                                              | **Port** (Task 3.2)                                                                |
...
| Duplicate profile                                                          | `ProfileActions.tsx:119-126`                                               | Missing                                                              | **Port** (Tasks 1.2 + 3.2)                                                         |
| Rename profile (+modal, undo toast, F2)                                    | `ProfileActions.tsx:127-134`; `useProfilesPageNotifications.ts:95-154`     | Missing (rename-pause autosave guard exists)                         | **Port** (Tasks 1.2 + 3.2)                                                         |
| Delete profile (+confirm overlay, collection-aware)                        | `ProfileActions.tsx:169-176`; `ProfilesPage.tsx:296-318`                   | Missing                                                              | **Port** (Tasks 1.2 + 3.2)                                                         |
| TOML preview                                                               | `ProfileActions.tsx:135-142`; `ProfilesPage.tsx:276-280`                   | Missing                                                              | **Port** (Tasks 1.2 + 3.2)                                                         |
| Community export                                                           | `ProfileActions.tsx:143-150`; `ProfilesPage.tsx:281-290`                   | Missing                                                              | **Port** (Tasks 1.2 + 3.2)                                                         |
| Config history / rollback / mark known good                                | `ProfileActions.tsx:161-168`; `useProfileHistory.ts`                       | Missing (Hero History tab = _launch_ history, distinct)              | **Port** (Tasks 1.2 + 3.2)                                                         |
| Mark as Verified (version drift ack)                                       | `ProfileActions.tsx:151-160`                                               | Missing                                                              | **Port** (Tasks 1.2 + 3.2)                                                         |
| Prefix dependencies panel                                                  | `PrefixDepsPanel` (`ProfilesPage.tsx:163-171`)                             | Missing                                                              | **Port** (Task 2.2, inside `CollapsibleSection`)                                   |
| Runtime suggestion banner (community-recommended Proton, ProtonUp install) | `ProfilesPage.tsx:173-217`; `useProfilesPageProton.ts:87-131`              | Bare `useProtonInstalls` only                                        | **Port** (Task 2.2)                                                                |
| Profile health badge (click→issues) + health issues list                   | `ProfilesPage.tsx:55-99,158-161`                                           | Per-card `HealthBadge` only; Overview has `GameDetailsHealthSection` | **Port** (Task 2.2 — issues list in editor; badge click scrolls to it)             |
...
| Trainer type chip / version status badge / network isolation badge         | `ProfilesPage.tsx:28-46,129-142`                                           | Missing                                                              | **Port** (Task 2.2 — render with trainer/health affordances)                       |
...
| Stale-check note ("Checked Nd ago")                                        | `ProfilesPage.tsx:149-153`                                                 | Missing                                                              | **Port** (Task 2.2 — alongside health issues list)                                 |
...
- [ ] Launch parity inventory documents every legacy Launch route sub-tab/option as ported / intentionally omitted (superseded) / superseded — finalized in Task 5.2.
- [ ] Profiles parity inventory ditto.
- [ ] Hero Detail `launch-options` exposes all legacy Launch route user-facing controls that would otherwise be lost when `/launch` is removed.
- [ ] Hero Detail `profiles` exposes all legacy Profiles route user-facing controls that would otherwise be lost when `/profiles` is removed.
- [ ] Existing Launch/Profile route behavior remains working until Phase 10 deletion (legacy test packs green; behavior-neutral extractions).
- [ ] No new package dependency added for parity UI.
- [ ] `npm run typecheck`, focused Launch/Profile tests, and full frontend tests pass.
- [ ] Responsive checks verify no horizontal overflow in Hero Detail Launch/Profile parity surfaces (new Playwright assertions, Task 5.1).
