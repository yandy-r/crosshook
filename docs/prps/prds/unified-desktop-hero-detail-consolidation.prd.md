...
| --- | ------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | -------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Hero Detail panel contract expansion + test harness                                  | Extend `HeroDetailPanelsProps` with `profile`, `updateProfile`, `profileList`, `onSetActiveTab`. Thread through `GameDetail` + `HeroDetailTabs`. Add fixtures. No UI change.                  | complete    | -        | -       | [`hero-detail-panel-contract-phase-1.plan.md`](../plans/hero-detail-panel-contract-phase-1.plan.md)                                                                                      |
...
| 3   | Pre/post hook schema (crosshook-core additive fields + serde round-trip test)        | Add `LaunchHook`, `HookStage`, `pre_launch_hooks`, `post_exit_hooks` to profile struct. `cargo test` round-trip. No launcher consumption yet.                                                 | complete    | with 2   | -       | [`github-issue-468-launch-hooks-schema.plan.md`](../plans/completed/github-issue-468-launch-hooks-schema.plan.md)                                                                        |
| 4   | Hero Detail Profiles tab (two-pane editor)                                           | New `HeroDetailProfilesTab.tsx`. Left list + flattened editor reusing `profile-sections/*`. `selectProfile()` on card click. Replace `ProfilesPanel` in panels switch. Autosave parity tests. | in-progress | -        | 1       | [`github-issue-469-hero-detail-profiles-tab.plan.md`](../plans/github-issue-469-hero-detail-profiles-tab.plan.md)                                                                        |
| 5   | Hero Detail Launch tab (3-section stack) + highlighted command preview               | New `HeroDetailLaunchTab.tsx` + `HighlightedCommandBlock.tsx`. Reuse `CustomEnvironmentVariablesSection`. Hook panel stub (no hook data yet). Replace `launch-options` panel.                 | complete    | with 4   | 1       | [`github-issue-470-hero-detail-launch-tab.plan.md`](../plans/completed/github-issue-470-hero-detail-launch-tab.plan.md)                                                                  |
...
| 7   | Overview tab deep-links                                                              | Add "Edit launch config →" / "Open profile →" / "Manage hooks →" / "Edit env vars →" buttons that call `onSetActiveTab`.                                                                      | complete    | with 6   | 4, 5    | [`plan`](../plans/completed/github-issue-472-overview-panel-deep-links.plan.md), [`report`](../reports/github-issue-472-overview-panel-deep-links-report.md)                             |
| 8   | `AppRoute` union shrink + TypeScript cleanup                                         | Remove `'profiles'` and `'launch'` from `AppRoute`. Let `tsc` enumerate every remaining reference. Prepare rewire checklist.                                                                  | complete    | -        | 4, 5    | Shipped in PR #495 (`9f72b4f5`)                                                                                                                                                          |
| 9   | Nav rewire — all onNavigate + palette handlers                                       | Update LibraryGrid/LibraryList/LibraryPage/HealthDashboardPage/InstallPage/CollectionViewModal/CollectionLaunchDefaultsEditor/AppShell palette to open Hero Detail + set tab.                 | complete    | -        | 8       | Shipped in PR #495 (`9f72b4f5`)                                                                                                                                                          |
| 10  | Route deletion — ProfilesPage, LaunchPage, subdirs, RTL tests, ContentArea cases     | Physical removal from git. Clean unused icons / banner assets. Ensure `npm test` stays green.                                                                                                 | complete    | -        | 9       | [`plan`](../plans/completed/github-issues-475-476-legacy-page-deletion-smoke-rewrite.plan.md), [`report`](../reports/github-issues-475-476-legacy-page-deletion-smoke-rewrite-report.md) |
| 11  | Playwright smoke rewrite — Hero Detail flows, new sidebar entries, regression guards | Remove old profiles/launch smoke blocks. Add Hero Detail launch flow, profiles edit flow, env toggle flow, Favorites + Currently Playing sidebar flows. Add `AppRoute` regression grep.       | complete    | -        | 10      | [`plan`](../plans/completed/github-issues-475-476-legacy-page-deletion-smoke-rewrite.plan.md), [`report`](../reports/github-issues-475-476-legacy-page-deletion-smoke-rewrite-report.md) |
