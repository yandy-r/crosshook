...
- [x] **Telemetry scope** — CrossHook today has no anonymous telemetry for launch outcomes. Establishing a baseline for the "issue decline" success metric may require building one, or the metric stays purely qualitative. Flagged but not required for PRD.
      **RESOLVED (2026-04-19, issue #246)**: Decision — do not build launch-outcome telemetry. Success metrics stay fully qualitative (GitHub issue label analytics + user-sentiment themes per PRD §Success Metrics). Rationale: primary metric does not need telemetry; power-user Linux audience has historically low opt-in + severe self-selection bias that would make the data worse than qualitative signal; cost of consent UX + event schema + upload endpoint + privacy policy + Flatpak network-permission broadening is not justified by a secondary metric with a manual-sampling fallback; no-telemetry is a positive product value for this audience. Reversible if a future gap in qualitative signal is specifically identified.
