...
- [ ] **Is `org.openwinecomponents.umu.umu-launcher` published on Flathub today?** Last confirmed state (upstream issue #335, Jan 2025) was "no Flathub listing." Upstream keeps an in-tree manifest. Must be re-checked at Phase 5 implementation start; determines whether CrossHook's "install UMU" dialog can offer a one-click Flathub action.
- [ ] **Exact `PROTONPATH` format** — umu accepts both a directory path and a tag name (`GE-Proton`, `GE-Proton9-20`). CrossHook stores the Proton executable path. Implementation must validate whether `dirname(proton_path)` is sufficient or whether a tag lookup is required for user-installed GE-Proton builds.
...
| --- | -------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ------- | -------- | -------------------- | -------- |
| 1   | PROTON_VERB hygiene                                | Set `PROTON_VERB=waitforexitandrun` for game builder, `runinprefix` for trainer builders; add to `WINE_ENV_VARS_TO_CLEAR`                | pending | -        | -                    | -        |
