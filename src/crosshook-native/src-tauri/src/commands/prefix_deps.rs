...
use crosshook_core::prefix_deps::detection::detect_binary;
use crosshook_core::prefix_deps::lock::PrefixDepsInstallLock;
use crosshook_core::prefix_deps::runner;
use crosshook_core::prefix_deps::{BinaryDetectionResult, DependencyState, PrefixDependencyStatus};
...
pub async fn check_prefix_dependencies(
    ...
    let binary_path = detection.binary_path.unwrap();

    // Run check
    let installed = runner::check_installed(&binary_path, &prefix_path)
        .await
...
pub async fn install_prefix_dependency(
    ...
    let mut child = runner::install_packages(&binary_path, &prefix_path, &packages, None)
        .await
        .map_err(|e| e.to_string())?;
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Some(stdout) = stdout {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                ...
                let _ = app_clone.emit("prefix-dep-log", DepLogPayload { line: clean });
            }
        }
    });
    ...
    tauri::async_runtime::spawn(async move {
        ...
        if let Some(stderr) = stderr {
            ...
            while let Ok(Some(line)) = lines.next_line().await {
                let clean = strip_ansi_codes(&line);
                let _ = app_clone2.emit("prefix-dep-log", DepLogPayload { line: clean });
            }
        }
    });
