...
pub fn build_dependency_health_issues(
    ...
    for verb in required_verbs {
        let state = dep_states
            .iter()
            .find(|row| row.package_name == *verb)
...
mod tests {
    ...
    // Prefix dependency health issue tests

    fn make_dep_row(package: &str, state: &str) -> PrefixDependencyStateRow {
    ...
    fn installed_verbs_produce_no_issues() {
        ...
        let issues = build_dependency_health_issues(&deps, &required);
