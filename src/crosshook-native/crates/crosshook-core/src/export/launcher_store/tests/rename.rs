...
#[test]
fn rename_refuses_to_overwrite_symlink_destination() {
    ...
    // Create a symlink at the new script path
    let new_slug = "new-game";
    ...
    let parent = Path::new(&new_script_path).parent().expect("parent");
    fs::create_dir_all(parent).expect("mkdir");
    unix_fs::symlink(&real_file, &new_script_path).expect("symlink");
    ...
    assert!(!result.script_renamed, "script should not be renamed");
    assert!(
        result.old_script_cleanup_warning.is_some(),
        "should have warning about refusing to overwrite"
    );
    assert!(result
    ...
    assert!(
        Path::new(&old_script).exists(),
        "old script should remain untouched"
    );

    // Symlink should still exist and point to the real file
    assert!(
        Path::new(&new_script_path).exists(),
        "symlink should remain"
    );
    let metadata = fs::symlink_metadata(&new_script_path).expect("get metadata");
    assert!(
...
#[test]
fn rename_refuses_to_overwrite_non_crosshook_file_at_destination() {
    let temp = tempdir().expect("temp dir");
    let home = temp.path().to_string_lossy().into_owned();
    let request = make_test_request();
    let old_slug = "old-game";
    ...
    // Create a non-CrossHook file at the new script path (no watermark)
    let new_slug = "new-game";
    let new_script_path = combine_host_unix_path(
        &home,
        ".local/share/crosshook/launchers",
    ...
    create_file_with_content(&new_script_path, "#!/usr/bin/env bash\necho unmanaged\n");
    ...
    assert!(result
        ...
        .as_ref()
        .unwrap()
        .contains("Missing CrossHook watermark"));

    // Old file should still exist
    assert!(Path::new(&old_script).exists(), "old script should remain");

    // Non-CrossHook file should remain untouched
    let content = fs::read_to_string(&new_script_path).expect("read file");
    assert!(
        content.contains("echo unmanaged"),
        "non-CrossHook file should remain unchanged"
    );
}

#[test]
fn rename_allows_overwriting_crosshook_file_at_destination() {
    let temp = tempdir().expect("temp dir");
    let home = temp.path().to_string_lossy().into_owned();
    let request = make_test_request();
    let old_slug = "old-game";
    ...
    // Create a CrossHook-managed file at the new script path (with watermark)
    let new_slug = "new-game";
    let new_script_path = combine_host_unix_path(
        &home,
        ".local/share/crosshook/launchers",
    ...
    create_watermarked_script(&new_script_path);

    let result = rename_launcher_files(old_slug, "New Game", "", &home, "", &request)
        .expect("rename should succeed");

    assert!(
        result.script_renamed,
        "script should be renamed/overwritten"
    );
    assert!(
        result.old_script_cleanup_warning.is_none(),
        "should have no warning"
    );

    // New file should exist with updated content
    let content = fs::read_to_string(&new_script_path).expect("read new script");
    assert!(
        content.contains("# New Game - Trainer launcher"),
        "new script should have updated content"
    );

    // Old file should be deleted (slug changed)
    assert!(
        !Path::new(&old_script).exists(),
        "old script should be removed"
    );
