...
fn build_trainer_script_content(
    ...
    content.push_str("exec \"$PROTON\" run \"$TRAINER_WINDOWS_PATH\"\n");
    content
}
...
mod tests {
    ...
    fn export_writes_expected_paths_and_content() {
        ...
        assert!(script_content
            .contains("TRAINER_WINDOWS_PATH='/opt/Trainers/Trainer'\"'\"'s Edition.exe'"));
        assert!(script_content.contains("exec \"$PROTON\" run \"$TRAINER_WINDOWS_PATH\""));
