...
import { findLaunchOptimizationConflicts, type LaunchOptimizationId } from '../types/launch-optimizations';
...
import OptionGroup from './launch-optimizations/OptionGroup';
import {
  type CapabilityId,
  formatCountLabel,
  type GroupedOptions,
  groupOptions,
  joinClasses,
} from './launch-optimizations/utils';
...
export function LaunchOptimizationsPanel({
  ...
  const commonGroups: GroupedOptions[] = groupOptions(commonOptions);
  const advancedGroups: GroupedOptions[] = groupOptions(advancedOptions);
