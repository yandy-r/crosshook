...
- **Issues**: Open all CrossHook issues on Forgejo (`origin`, `https://git.home.rfamily.dev/yandy/crosshook`) using `.forgejo/ISSUE_TEMPLATE/`. Do not create title-only or template-bypass issues. For feature issues that introduce or change **persisted** data, the issue body must include a **Storage boundary** subsection (TOML settings / SQLite metadata / runtime-only) and a short **Persistence & usability** subsection.
- **Pull requests**: Open all CrossHook PRs on Forgejo and follow `.forgejo/pull_request_template.md`. Always link the related Forgejo issue (`Closes #…`). Label PRs using the project taxonomy — never invent ad-hoc labels.
- **Releases**: Before tagging, run `./scripts/prepare-release.sh …` (regenerates/validates `CHANGELOG.md`). Collaboration branches push to Forgejo `origin`; release tags push to both Forgejo `origin` and GitHub `github` so each remote's release workflow publishes the Flatpak bundle.
