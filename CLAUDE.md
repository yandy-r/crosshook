...
- **Firewalled agent environments** (GitHub Copilot coding agent and similar CI): Outbound network is blocked at agent runtime except for the repo allowlist. Heavy installs (`apt-get install`, `cargo install`, `npm install`, `npx playwright install`, system Chrome from `dl.google.com`) **must** happen in [`.github/workflows/copilot-setup-steps.yml`](.github/workflows/copilot-setup-steps.yml)—which runs before the firewall engages—**not** at agent runtime. On `http block` / `dns block`: stop, name the blocked host, and escalate so setup-steps or the allowlist can be updated. **Never** blind-retry a firewalled call—blocks are deterministic and retries fail identically.
...
- **GitHub API fallback**: When `gh api` / `gh issue` / `gh pr` is blocked or fails, fall back to local git (`git log`, `git show`, `git notes`, `git branch -v`) or the GitHub MCP server (different auth path). Do **not** re-run the same failing `gh` call—diagnose the cause (firewall, auth, shell quoting) first.
