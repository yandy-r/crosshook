...
- **Releases**: Before tagging, run `./scripts/prepare-release.sh …` (regenerates/validates `CHANGELOG.md` per repo scripts). Collaboration branch pushes go to Forgejo `origin`; release tags go to the GitHub `github` remote so `.github/workflows/release.yml` can publish GitHub Releases. The release workflow validates changelog sections; noisy commits should fail prep, not ship to GitHub Releases.
...
- **CI**: `.forgejo/workflows/lint.yml` runs Rust, TypeScript, ShellCheck, and smoke checks on Forgejo PRs; `.github/workflows/release.yml` publishes GitHub Releases from tags.
